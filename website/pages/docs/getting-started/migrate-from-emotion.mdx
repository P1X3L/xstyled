...
```js
/** @jsx jsx */
import { jsx } from '@wttj/xstyled-emotion'
...
`@wttj/xstyled-emotion` is a replacement of `@emotion/react` and `@emotion/styled`. You can safely replace `@emotion/react` and `@emotion/styled` by `@wttj/xstyled-emotion`.
...
JSX Pragma is exposed in `@wttj/xstyled-emotion`:
...
`css` prop with `@wttj/xstyled-emotion` is slightly different from `css` prop with `@emotion/react`.
